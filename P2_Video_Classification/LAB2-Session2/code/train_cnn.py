...
def get_generators(folder_data, data):
    '''Creater generator in correcto formart for train/validating tasks in keras'''
    ...
    train_generator = train_datagen.flow_from_directory(
        os.path.join(folder_data, 'train'),
        target_size=(299, 299),
        batch_size=8,
...
def get_model(nb_classes,weights='imagenet'):
    '''
        Args:
            weights: init weights to initilizate net
            nb_classes: number of video classes
        Returns: 
            model
...
def train_model(model, nb_epoch, generators, callbacks=[]):
    '''Trains the model. It basically calls Keras fit fuction'''
...
def execute(weights_file,folder_data='data', class_limit = 5, seq_length = 5 ):
    '''Trains a CNN model. Saves best model at checkpoints folder with its log 
        within logs folder
    
        Args:
            weights_file(str):saved weights.It avoids the first fine tuning step(top brief training)
    ...
    checkpointer = ModelCheckpoint(
        filepath=os.path.join(folder_data, 'checkpoints', 'inception.{epoch:03d}-{val_loss:.2f}' + '-classes_' + str(nb_classes) + '_.hdf5'),
        verbose=1,
        save_best_only=True)
    ...
    # Helper: TensorBoard
    tensorboard = TensorBoard(log_dir=os.path.join(folder_data, 'logs', 'inception'))

    # Helper: Save results.
    timestamp = time.time()
    csv_logger = CSVLogger(os.path.join(folder_data, 'logs', 'inception' + '-' + 'classes_' + str(nb_classes) + '_training-' + str(timestamp) + '.log'))


    #Creater generators for the dataset
    model = get_model(nb_classes)
    generators = get_generators(folder_data,data)
